...
public class PS2DEditor:Editor{
	...
	void DrawPoint(int pointID){
		...
		//Change to default cursor if mouse if near the point
		Handles.color=Color.white;
		if(Vector2.Distance(script.transform.TransformPoint(script.points[pointID].position),GetMouseWorldPosition())<size){
	...
	void DrawBezierControls(int i){
		...
		//Bezier controls
			...
			if(script.points.Loop(i-1).selected || script.points[i].selected || showAllControls){
				...
				Handles.color=Color.white;
				Handles.DrawSolidDisc(script.transform.TransformPoint(script.points[i].handleP),Vector3.back,size*0.5f);
				Handles.color=Color.clear;
				EditorGUI.BeginChangeCheck();
				Vector3 handlePPoint=Handles.FreeMoveHandle(
					script.transform.TransformPoint(script.points[i].handleP),
					script.transform.rotation,
					size*0.5f,
					Vector3.zero,
					CircleHandleCapSaveID
				...
				//Assign last control id (retrieved by CircleHandleCapSaveID function)
				//Those events are excluded because they cause the number to be the same for all points
				...
				if(EditorGUI.EndChangeCheck()){
					...
					script.points[i].handleP=script.transform.InverseTransformPoint(handlePPoint);
					script.UpdateMesh();
				}
				//Find if mouse is near this handle
				if(Vector2.Distance(script.transform.TransformPoint(script.points[i].handleP),GetMouseWorldPosition())<=size*1.5f){
			...
			if(script.points.Loop(i+1).selected || script.points[i].selected || showAllControls){
				...
				Handles.color=Color.white;
				Handles.DrawSolidDisc(script.transform.TransformPoint(script.points[i].handleN),Vector3.back,size*0.5f);
				Handles.color=Color.clear;
				EditorGUI.BeginChangeCheck();
				Vector3 handleNPoint=Handles.FreeMoveHandle(
					script.transform.TransformPoint(script.points[i].handleN),
					script.transform.rotation,
					size*0.5f,
					Vector3.zero,
					CircleHandleCapSaveID
				...
				if(EditorGUI.EndChangeCheck()){
					if(!isDraggingPoint) isDraggingPoint=true;
					Undo.RecordObject(script,"Edit point");
					script.points[i].handleN=script.transform.InverseTransformPoint(handleNPoint);
					//Move other bezier handle accordingly
					if(script.points[i].pointType==PS2DPointType.Rounded){
				...
				//Find if mouse is near this handle
				if(Vector2.Distance(script.transform.TransformPoint(script.points[i].handleN),GetMouseWorldPosition())<=size*1.5f){
					dontDrawAddPoint=true;
				}
				drawControls--;
			}
